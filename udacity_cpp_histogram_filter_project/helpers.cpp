...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	for (int i=0; i<height; i++) {
		for (int j=0; j<width; j++) {
			...
			newGrid[i][Idx((j-1), width)]+=adjacent_prob*grid[i][j];
			newGrid[i][Idx((j+1), width)]+=adjacent_prob*grid[i][j];
			newGrid[Idx((i-1), height)][j]+=adjacent_prob*grid[i][j];
			newGrid[Idx((i+1), height)][j]+=adjacent_prob*grid[i][j];
			newGrid[Idx((i-1), height)][Idx((j-1), width)]+=corner_prob*grid[i][j];
			newGrid[Idx((i-1), height)][Idx((j+1), width)]+=corner_prob*grid[i][j];
			newGrid[Idx((i+1), height)][Idx((j+1), width)]+=corner_prob*grid[i][j];
			newGrid[Idx((i+1), height)][Idx((j-1), width)]+=corner_prob*grid[i][j];
		}
